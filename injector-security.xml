...
people who do care about security, this document explains how Zero Install
...
<dl class='spaced'>
 ...
 Zero Install only runs code that is signed by a GPG key you trust to provide software from that web-site. By default, it will automatically approve keys known to the key information server; this can be disabled from the preferences dialog.
 </dd>
...
<p>
For a demonstration of using sandboxing with Zero Install, see <a href='ebox.html'>ebox</a>.
</p>

<h2>Authorisation vs Privileges</h2>

<p>
It's also important to separate out two aspects of installation that are easily
confused: <i>who</i> is authorised to install software and <i>what privileges</i> the software
is given by the act of installing it. Here, "apt-get" is used as a short-hard
for "a typical traditional Linux package manager":
</p>

<table class='table'>
<tr><th></th><th colspan='2'>Who can install things</th></tr>
<tr><th>Privileges granted</th><th>Only root</th><th>Anyone</th></tr>
<tr><th>Full access</th><td>Default apt-get</td><td>apt-get with modified rules</td></tr>
<tr><th>No access</th><td>Zero Install with restrictions</td><td>Default Zero Install</td></tr>
</table>

<p>
A typical package manager only allows root (or an administrator) to install
software by default, and grants that software full access to the machine,
including access to all user accounts. You can change the rules to allow others to
install software (e.g. using PolicyKit or sudoers, you might allow anyone to upgrade a
package), but the software still gets complete access to the machine.
</p>

<p>
Zero Install allows anyone to install software by default, but gives the
software no access to the machine (since it doesn't run any of its code at
install time). You could configure Zero Install so that only root can install software,
but the software would still get no ability to do anything at install time.
</p>

<p>
The reason we allow anyone to install software by default is the principle of
<i>Do Not Prohibit What You Cannot Prevent</i> [citation needed]. On Linux, unprivileged
users can install software even without Zero Install, so trying to stop them makes life
harder for legitimate users but doesn't improve security.
</p>
...
<h2>Zero Install's trust model</h2>
...
<p>
The basis of Zero Install's model is the standard multi-user model used by
...
upgraded when a problem is found (e.g. in the Linux kernel). However, if users
...
user runs a program, Zero Install downloads the feed and checks that it
...
<p>
Once trusted, Zero Install will accept any feed from the same domain
signed by that key.
</p>

<p>
Zero Install updates feeds at the request of the user (clicking the
'Refresh' button), or automatically (monthly, by default) if the
user sets their freshness policy to request this. If a program is not
used, Zero Install does not check for updates to it.
</p>

<p>
Zero Install does not accept incoming network connections, so even if a malicious key
...
has come from and to only accept digitally signed software. Zero Install maintains
