...
:   Installers typically ship compressed payloads that are expanded on disk; `--collect-files` archives the expanded layout. You can shrink the result by re-compressing with a stronger algorithm (e.g. 7z) or by hosting the original installer instead and pointing the `<archive>` at it with `type="application/x-msi"` (see [Native Windows apps](../packaging/guides/windows-native.md#msi-installers)).
