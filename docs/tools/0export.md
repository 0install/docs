...
Unlike the bundles created by the related [Zero2Bundle](http://roscidus.com/desktop/Zero2Bundle) program, programs installed using `setup.sh` are added to the Zero Install cache and are therefore still [shared between users](../details/sharing.md), and will get updates over the web where possible.
...
This program is not yet complete. In particular, adding the program to the user's menus only works if the user already has Zero Install itself installed.
