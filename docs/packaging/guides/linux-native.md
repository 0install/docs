...
!!! tip
    When upstream doesn't publish a generic tarball, an RPM often works on Debian (and vice versa). If several distro-specific packages are available, pick the one for the oldest distribution: packages built against an older glibc keep working on newer systems, but not the other way around.
...
!!! attention
    ...
    Shell-script launchers that `cd` to a hard-coded directory are the same problem in miniature. The cleanest fix is upstream (`cd "$(dirname "$0")"` is portable); if that's not an option, use a `<recipe>` to patch the script, and compute the post-patch implementation ID with `0install digest` on the patched tree.
