...
These guides are reference recipes for packaging specific kinds of software with Zero Install. They assume you've already read [Concepts](../concepts.md) and skimmed the [publishing tutorial](../tutorials/publish-app.md), and focus on the parts of the feed that differ between platforms and runtimes: which `arch`, which `<command>`, which dependencies, which environment bindings.
