...
if os.system('xgettext --keyword=_ --from-code=utf-8 feed-translations.c -o feed-translations.tmp'):
	raise Exception('xgettext failed')
os.unlink('feed-translations.c')

data = file('feed-translations.tmp').read()
while data.startswith('#'):
	data = data[data.index('\n') + 1:]
	
data = '''# Zero Install Feed Translations
# Copyright owned by individual feed authors

''' + data

data = data.replace("Project-Id-Version: PACKAGE VERSION",
		    "Project-Id-Version: Zero Install Feed Translations")

with file('feed-translations.po', 'w') as s:
	s.write(data)
os.unlink('feed-translations.tmp')
